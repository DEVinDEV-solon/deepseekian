...
function setShims(shims2, options = { auto: false }) {
  ...
  kind = shims2.kind;
  fetch2 = shims2.fetch;
  Request2 = shims2.Request;
  Response2 = shims2.Response;
  Headers2 = shims2.Headers;
  FormData2 = shims2.FormData;
  Blob2 = shims2.Blob;
  File2 = shims2.File;
  ReadableStream2 = shims2.ReadableStream;
...
var DeepSeekAPI = class {
  ...
  async chat(message, systemPrompt) {
    if (!this.client) {
      throw new Error("API-Schl\xFCssel nicht konfiguriert");
    }
    try {
  ...
  async reasonerChat(message, systemPrompt) {
    if (!this.client) {
      throw new Error("API-Schl\xFCssel nicht konfiguriert");
    }
    try {
...
var DeepSeekianSettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian.Setting(containerEl).setName("API-Schl\xFCssel").setDesc("Dein DeepSeek API-Key (sk-...)").addText((text) => text.setPlaceholder("sk-...").setValue(this.plugin.settings.apiKey).onChange(async (value) => {
      this.plugin.settings.apiKey = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian.Setting(containerEl).setName("API Base URL").setDesc("DeepSeek API Endpunkt").addText((text) => text.setValue(this.plugin.settings.apiBaseUrl).onChange(async (value) => {
      this.plugin.settings.apiBaseUrl = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian.Setting(containerEl).setName("Standard-Modell").setDesc("W\xE4hle das Standard-Modell").addDropdown((dropdown) => dropdown.addOption("deepseek-chat", "DeepSeek Chat (V3)").addOption("deepseek-reasoner", "DeepSeek Reasoner (R1)").addOption("deepseek-coder", "DeepSeek Coder").setValue(this.plugin.settings.model).onChange(async (value) => {
      this.plugin.settings.model = value;
      await this.plugin.saveSettings();
    }));
    containerEl.createEl("hr");
    containerEl.createEl("p", {
      text: "\u{1F4A1} Tipp: Hole einen kostenlosen API-Key auf https://platform.deepseek.com",
      cls: "deepseekian-tip"
    });
  }
};
...
var VIEW_TYPE_DEEPSEEKIAN = "deepseekian-view";
var DeepSeekianView = class extends import_obsidian2.ItemView {
  constructor(leaf, plugin) {
    super(leaf);
    this.selectedSkill = "deepseek-chat";
  ...
  async onOpen() {
    ...
    skillContainer.createEl("label", { text: "W\xE4hle einen Skill:" });
    const skillSelect = skillContainer.createEl("select");
    ...
    const sendBtn = buttonGroup.createEl("button", { text: "\u{1F4E4} Senden" });
    sendBtn.addEventListener("click", () => this.sendRequest());
    const insertBtn = buttonGroup.createEl("button", { text: "\u270F\uFE0F Einf\xFCgen" });
    insertBtn.addEventListener("click", () => this.insertResponse());
    const clearBtn = buttonGroup.createEl("button", { text: "\u{1F5D1}\uFE0F L\xF6schen" });
  ...
  async sendRequest() {
    if (!this.promptInput || !this.promptInput.value.trim()) {
      new import_obsidian2.Notice("\u274C Bitte gib eine Anfrage ein");
      return;
    }
    if (!this.plugin.settings.apiKey) {
      new import_obsidian2.Notice("\u274C API-Schl\xFCssel nicht konfiguriert. Gehe zu Plugin-Einstellungen.");
      return;
    }
    this.isLoading = true;
    ...
    try {
      ...
      switch (this.selectedSkill) {
        ...
        case "deepseek-obsidian":
          response = await api.chat(this.promptInput.value, "Du bist ein Assistent f\xFCr Obsidian-Notizen. Helfe beim Organisieren, Zusammenfassen und Verkn\xFCpfen von Wissen.");
          break;
        default:
      ...
      new import_obsidian2.Notice("\u2705 Anfrage verarbeitet!");
    } catch (error) {
      if (this.responseContainer) {
        this.responseContainer.innerHTML = `<div class="deepseekian-error">\u274C Fehler: ${error}</div>`;
      }
      new import_obsidian2.Notice(`\u274C Fehler: ${error}`);
    } finally {
      this.isLoading = false;
    }
  }
  insertResponse() {
    ...
    if (responseText === "Keine Antwort noch." || !responseText) {
      new import_obsidian2.Notice("\u274C Keine Antwort zum Einf\xFCgen");
      return;
    }
    const activeEditor = this.app.workspace.activeEditor?.editor;
    if (!activeEditor) {
      new import_obsidian2.Notice("\u274C Kein aktiver Editor. \xD6ffne eine Notiz.");
      return;
    }
...
`);
    new import_obsidian2.Notice("\u2705 Antwort eingef\xFCgt!");
  }
  clearResponses() {
    if (this.responseContainer) {
      this.responseContainer.textContent = "Keine Antwort noch.";
    ...
    new import_obsidian2.Notice("\u{1F5D1}\uFE0F Gel\xF6scht");
  }
  escapeHtml(text) {
    const map = {
      "&": "&amp;",
...
var DeepSeekianPlugin = class extends import_obsidian3.Plugin {
  constructor() {
    super(...arguments);
  ...
  async onload() {
    ...
    const ribbonIconEl = this.addRibbonIcon("brain", "DeepSeekian", (evt) => {
      this.activateView();
    });
    ...
    this.addCommand({
      ...
      editorCallback: (editor) => {
        new import_obsidian3.Notice("DeepSeek Chat ge\xF6ffnet. Nutze die Sidebar.");
      }
    });
    this.addCommand({
      ...
      editorCallback: (editor) => {
        new import_obsidian3.Notice("DeepSeek Reasoner aktiviert. Nutze die Sidebar.");
      }
    });
    this.addCommand({
      ...
      editorCallback: (editor) => {
        new import_obsidian3.Notice("DeepSeek Coder aktiviert. Nutze die Sidebar.");
      }
    });
    this.addCommand({
      ...
      editorCallback: (editor) => {
        new import_obsidian3.Notice("DeepSeek Obsidian-Werkzeuge ge\xF6ffnet. Nutze die Sidebar.");
      }
    });
